...
async def read_root(
    ...
    try:
        # Get user context
        user_context = await enhanced_auth(session)
    ...
    logger.info(f"User {user_context['username']} accessible_services: {accessible_services}")
    logger.info(
...
async def internal_register_service(
    ...
    username = await verify_admin_credentials(request)
    ...
    logger.debug(f"INTERNAL REGISTER: Created server entry: {server_entry}")
    ...
    if not success:
        logger.debug(f"INTERNAL REGISTER: Registration failed for path {path}")
        return JSONResponse(
            status_code=409,  # Conflict status code for existing resource
            content={
                "error": "Service registration failed",
    ...
    logger.debug("INTERNAL REGISTER: Server registered successfully, adding to FAISS index")

    # Add to FAISS index with current enabled state (should be True after auto-enable)
    is_enabled = await server_service.is_service_enabled(path)
    await faiss_service.add_or_update_service(path, server_entry, is_enabled)

    logger.debug("INTERNAL REGISTER: Regenerating Nginx configuration")
    ...
    logger.debug("INTERNAL REGISTER: Updating scopes.yml for new server")
    ...
    tool_names: list[str] = []
    entry_tool_list: Any = server_entry.get("tool_list")
    if entry_tool_list and isinstance(entry_tool_list, list):
        tool_names = [
            tool["name"] for tool in entry_tool_list if isinstance(tool, dict) and "name" in tool
        ]
...
async def internal_remove_service(
    ...
    logger.debug("INTERNAL REMOVE: Service removed successfully, updating FAISS index")
    ...
    logger.debug("INTERNAL REMOVE: Removing server from scopes.yml")
...
async def internal_toggle_service(
    ...
    # Verify admin credentials
    username = await verify_admin_credentials(request)
    logger.debug(f"INTERNAL TOGGLE: Admin authentication successful for user '{username}'")
...
async def internal_healthcheck(request: Request):
    ...
    # Verify admin credentials
    username = await verify_admin_credentials(request)
    logger.debug(f"INTERNAL HEALTHCHECK: Admin authenticated successfully: {username}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve health status: {e!s}")
...
async def get_service_tools(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching tools: {e!s}")
...
async def internal_add_server_to_groups(
    ...
    # Verify admin credentials
    username = await verify_admin_credentials(request)
    logger.info(f"Adding server to groups via internal endpoint by admin '{username}'")
...
async def internal_remove_server_from_groups(
    ...
    # Verify admin credentials
    username = await verify_admin_credentials(request)
    logger.info(f"Removing server from groups via internal endpoint by admin '{username}'")
...
async def internal_list_services(
    ...
    # Verify admin credentials
    username = await verify_admin_credentials(request)
    logger.debug(f"INTERNAL LIST: Authentication successful for admin user '{username}'")
...
async def register_service_api(
    ...
    **Example:**
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Service registration failed: {e!s}")
    ...
    try:
        ...
        if create_in_idp:
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Failed to create group in IdP: {e!s}",
                )
...
async def rate_server(
    ...
    try:
        avg_rating = await server_service.update_rating(
            actual_path, user_context["username"], request.rating
        )
...
async def rescan_server(
    ...
    **Example:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to scan server: {e!s}",
        )
...
@router.get("/servers-debug")
async def get_servers_json_debug(
    request: Request,
    query: str | None = None,
    user_context: Annotated[dict, Depends(nginx_proxied_auth)] = None,
):
    """Get servers data as JSON with detailed debug logging (for troubleshooting)."""
    # CRITICAL DIAGNOSTIC: Log that we reached this endpoint
    logger.info(
        f"[GET_SERVERS_ENTRY] GET /api/servers-debug called from {request.client.host if request.client else 'unknown'}"
    )
    ...
    if user_context:
        ...
        logger.info(
            f"[GET_SERVERS_DEBUG] Accessible services: {user_context.get('accessible_services', 'NOT PRESENT')}"
        )
