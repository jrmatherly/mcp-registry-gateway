...
async def extract_auth_context(ctx: Context) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            auth_context["http_request_available"] = False
            auth_context["auth_headers"] = f"Not in HTTP context: {e!s}"
        except Exception as http_error:
            logger.debug(f"Could not access HTTP request: {http_error}")
            auth_context["http_request_available"] = False
            auth_context["auth_headers"] = f"HTTP access error: {http_error!s}"
        ...
        return {
            "error": f"Failed to extract auth context: {e!s}",
...
async def health_check(request):
    ...
    return JSONResponse({"status": "healthy", "service": "mcpgw-server"})
...
async def _call_registry_api(
    ...
    auth = (
        httpx.BasicAuth(registry_username, registry_password)
...
async def debug_auth_context(ctx: Context = None) -> dict[str, Any]:
    ...
    for prop in ["client_id", "request_id", "session", "request_context", "fastmcp"]:
        try:
            ...
            debug_info["context_properties"][prop] = f"ERROR: {e!s}"
...
async def get_http_headers(ctx: Context = None) -> dict[str, Any]:
    ...
    try:
        ...
        result.update(
            {
                ...
                "error": f"Not in HTTP context: {e!s}",
        ...
        result.update(
            {"http_request_available": False, "error": f"Error accessing HTTP request: {e!s}"}
        )
...
async def print_all_http_headers(ctx: Context = None) -> str:
    ...
    try:
        ...
        output.append(f"Not in HTTP context: {e!s}")
        output.append("This is expected for STDIO transport")
        await ctx.info(f"Not in HTTP context - this is expected for STDIO transport: {e}")

    except Exception as e:
        output.append(f"Error accessing HTTP request: {e!s}")
        await ctx.error(f"Error accessing HTTP request: {e}")
...
async def list_services(ctx: Context = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"services": [], "total_count": 0, "error": f"Failed to retrieve services: {e!s}"}
...
async def healthcheck(ctx: Context = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise Exception(f"Failed to retrieve health status: {e!s}")
...
async def add_server_to_scopes_groups(
    ...
    try:
        ...
        error_msg = f"Error adding server {server_name} to groups {group_names}: {e!s}"
...
async def remove_server_from_scopes_groups(
    ...
    try:
        ...
        error_msg = f"Error removing server {server_name} from groups {group_names}: {e!s}"
        logger.error(error_msg, exc_info=True)
        return {"success": False, "error": error_msg}
...
async def create_group(
    ...
    try:
        ...
        error_msg = f"Error creating group {group_name}: {e!s}"
        logger.error(error_msg, exc_info=True)
        return {"success": False, "error": error_msg}
...
async def delete_group(
    ...
    try:
        ...
        error_msg = f"Error deleting group {group_name}: {e!s}"
        logger.error(error_msg, exc_info=True)
        return {"success": False, "error": error_msg}
...
async def list_groups(ctx: Context = None) -> dict[str, Any]:
    ...
    try:
        ...
        error_msg = f"Error listing groups: {e!s}"
        logger.error(error_msg, exc_info=True)
        return {"success": False, "error": error_msg}
