...
import { API_ENDPOINTS, SEMANTIC_SEARCH_DEFAULTS } from "../../src/constants";
...
const waitForDebounce = async (): Promise<void> => {
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, SEMANTIC_SEARCH_DEFAULTS.DEBOUNCE_MS + 50),
  );
};
...
describe("useSemanticSearch", () => {
  ...
  describe("debouncing", () => {
    ...
    it("resets debounce timer on each keystroke", async () => {
      ...
      const { rerender } = renderHook(({ query }) => useSemanticSearch(query), {
        initialProps: { query: "te" },
      });

      // Wait a bit (less than full debounce)
      await new Promise((resolve) =>
        setTimeout(resolve, SEMANTIC_SEARCH_DEFAULTS.DEBOUNCE_MS / 2),
      );
      ...
      await waitFor(
        () => {
          ...
          expect(mockedAxios.post.mock.calls.length).toBeGreaterThan(
            callCountBefore,
          );
        },
        { timeout: SEMANTIC_SEARCH_DEFAULTS.DEBOUNCE_MS + 500 },
      ...
      const lastCall = mockedAxios.post.mock.calls[
        mockedAxios.post.mock.calls.length - 1
      ] as [string, { query: string }];
  ...
  describe("API calls", () => {
    ...
    it("sets loading state during API call", async () => {
      // Use a promise we can control with a resolver holder object
      const resolverHolder: { resolve?: (value: { data: SemanticSearchResponse }) => void } = {};
      const pendingPromise = new Promise<{ data: SemanticSearchResponse }>(
        (resolve) => {
      ...
      // Resolve the promise - resolver is guaranteed to be set at this point
      if (resolverHolder.resolve) {
        resolverHolder.resolve({ data: createMockResponse() });
      }
  ...
  describe("cleanup", () => {
    it("cancels in-flight request on query change", async () => {
      // First request - slow (controlled promise) with resolver holder object
      const resolverHolder: { resolve?: (value: { data: SemanticSearchResponse }) => void } = {};
      ...
      if (resolverHolder.resolve) {
        resolverHolder.resolve({ data: createMockResponse({ query: "first" }) });
      }
