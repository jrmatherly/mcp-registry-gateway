...
describe('constants', () => {
  ...
  describe('getScopeDescription', () => {
    it('returns description for known scopes', () => {
      ...
      expect(getScopeDescription('mcp-servers/write')).toBe('Write access to MCP servers')
    })
    ...
    it('returns default message for unknown scopes', () => {
      ...
      expect(getScopeDescription('random-string-123')).toBe('Custom permission scope')
    })
  })
